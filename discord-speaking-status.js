listenForDiscordEvents = function (e) {
  let user = game.users.find(u=> u.flags["discord-speaking-status"]?.id == e.data.userId)
  /*
  if (!user) {
    ...
    new Dialog({
      ...
      buttons: {
        confirm: {label:'confirm', callback: (html)=>{
          game.users.get(html.find('select').val()).setFlag('discord-speaking-status', 'id', e.data.userId)
        }}
      },
      default: 'confirm'
    },{id}).render(true)
...
openDiscordWindow = async function () {
  ...
  new Dialog({title:'Companion Script', content: `<p>copy this<br> paste it into the console (F12) of the discord window<br>execute to monitor speaking status</p><textarea>
